...
(defcustom pyim-title "灵通"
  "Pyim 在 mode-line 中显示的名称."
  :group 'pyim
  :type 'string)

(defcustom pyim-message-verbose t
  "Pyim 是否输出运行信息"
  :group 'pyim
  :type 'boolean)
...
(defun pyim-log (FMT &rest ARGS)
  "根据配置决定是否将信息输出到message中"
  (if pyim-message-verbose
      (message FMT ARGS)))
...
这个函数用于制作 elpa 格式的词库 ，不建议普通用户使用。"
  (let (replace result)
    ...
    (pyim-log "Add pyim dict %S to `pyim-extra-dicts'." (plist-get new-dict :name))
    t))
